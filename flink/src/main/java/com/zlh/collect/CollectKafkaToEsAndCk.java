...
import com.zlh.function.ClickhouseSinkFunction;
 ...
 */
@Slf4j
public class CollectKafkaToEsAndCk {
    public static void main(String[] args) {
        /**
         * job.name、parallelism、bootstrap.servers、zookeeper.connect、kafka.topic、es.host、es.index、ck.tableName、ck.ips、ck.tableColums
         */
        ...
        String tableName = params.get("ck.tableName");
        String account = params.get("ck.account");
        String passWd = params.get("ck.passWd");
        String[] ips = params.get("ck.ips").split(",");
        String[] tableColums = params.get("ck.tableColums").split(",");
        //flink运行环境
        ...
        try {
            ...
            // ck sink
            kafkaStream.addSink(new ClickhouseSinkFunction(tableName,ips,tableColums,null,null,account,passWd))
                    .setParallelism(parallelism).name("Clickhouse.Sink");
